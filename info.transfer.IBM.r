...
info.transfer.IBM <- function(d = d,   # these are the starting params of the population including birth and death rates. must have 5 columns: "age","birthRate","survivalRate","N0_proportion","N0_prob_knowing"
  ...
  # 4. Think about how to plot results, mainly what are the outputs were looking at to compare? e.g., time to 75% K? and % informed? We did already develop some code for this.
  # 5. 
  ...
  for(i in seq(ind)){
    ...
    ind[[i]]$informed <- rbinom(1,1, d$N0_prob_knowing[d$age == ind[[i]]$age])  # sample whether the animal knows the info
  ...
  for(i in seq(t)){ # loop for each time increment
    ...
    if(length(is.alive) > 1){   # is there anyone else left in the population?
      if(length(is.alive)/K < .75){  # has the population reached 75% of K? 
        ...
        for(j in is.alive){ #loop for each alive individual
          ...
          # now we need to update the interacting individuals (other than the current individual) and their info status
        ...
        # update the tosave data frame ready to write out
        tosave <- rbind(tosave, data.frame(time.stamp=Sys.time(), 
                                           t=i, 
