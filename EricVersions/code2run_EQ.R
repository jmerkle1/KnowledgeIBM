...
#setWD on merav's computer
#setwd("F:/KnowledgeIBM_TuningResults")


# Eric's folder paths
#in eric's computer
# box_fldr <- "~/Merav/KnowledgeIBM_TuningResults"
# on merav's computer
#box_fldr <- "F:/KnowledgeIBM_TuningResults"
...
dbConn <- dbConnect(SQLite(), paste0(box_fldr, "./KnowledgeIBM_ModelTuningK_20230207.sqlite"))
dbConnSup <- dbConnect(SQLite(), paste0(box_fldr, "./KnowledgeIBM_ModelTuningSupplemental_20230207.sqlite"))
...
#Eric's code
paramDF<- dbSendQuery(dbConn, "SELECT * FROM tblModelScenarios_EQ") %>% 
  dbFetch() %>% 
  mutate(vertTransmission = case_when(vertTransmission == 1 ~ TRUE,
                                      TRUE ~ FALSE))

#paramDF<- readxl::read_excel("./EricVersions/paramDF_043021.xlsx")
...
for(i in 1:nrow(paramDF)){
  ...
  dbWriteTable(dbConn, name = "tblModelStats_EQ", value = wStat, append = TRUE)
...
# ##the below lines of code aren't working
#  # result.folder = paste0(box_fldr,"/Results"), #an empty folder where results will be saved.
...
# pulled this out to try and work it but no output gets saved 
# result.folder = paste0(box_fldr,"/Results")

# Some quick plotting code
# results <- read.csv(paste0(box_fldr,"/Results/population_stats.csv"))
